-- Map the habit IDs from your check-ins to their titles
-- This will show us exactly what habits are stored for each day

-- Day 2 habits (9 habits completed)
SELECT 
    'Day 2' as day,
    h.title,
    h.category,
    h.points,
    CASE 
        WHEN th.is_core = true THEN 'CORE'
        ELSE 'BONUS'
    END as habit_type
FROM public.sz_habits h
LEFT JOIN public.sz_template_habits th ON th.habit_id = h.id 
...
-- Day 1 habits (4 habits completed)  
SELECT 
    'Day 1' as day,
    h.title,
    h.category,
