-- Add debugging to sz_create_streak function
...
BEGIN
    ...
    RAISE NOTICE 'Looking for template with key: %', v_template_key;
    
    select id into v_template_id from public.sz_templates where key = v_template_key;
    
    -- Debug: Log the result
    RAISE NOTICE 'Found template_id: %', v_template_id;
    ...
    if v_template_id is not null then
        RAISE NOTICE 'Assigning core habits from template_id: %', v_template_id;
        ...
        RAISE NOTICE 'No template_id found, using legacy behavior for mode: %', p_mode;
