-- =====================================================
-- FIX ONLY THE CHECK-IN FUNCTION UUID COMPARISON ERROR
-- =====================================================
-- This script fixes ONLY the sz_checkin function
-- =====================================================
...
BEGIN
    ...
    IF v_template_id IS NOT NULL THEN
        -- Use new template system - FIXED UUID comparison
        SELECT 
            COALESCE(SUM(
                CASE 
    ...
    IF v_existing_checkin_id IS NOT NULL THEN
        IF v_template_id IS NOT NULL THEN
            -- Use new template system for existing checkin - FIXED UUID comparison
            SELECT COALESCE(SUM(
                CASE 
                    WHEN th.is_core = false OR th.is_core IS NULL THEN 
                        COALESCE(th.points_override, h.points, 0)
...
-- Test the function
SELECT 'Check-in function fixed - UUID comparison error resolved' as status;
