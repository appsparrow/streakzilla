-- =====================================================
-- FIX UUID COMPARISON ERROR AND UPDATE STREAKS
-- =====================================================
-- This script fixes the "operator does not exist: uuid = text" error
-- and properly updates existing streaks
-- =====================================================

-- =====================================================
-- STEP 1: FIX CHECK-IN FUNCTION (AMBIGUOUS COLUMN ERROR)
-- =====================================================
...
-- =====================================================
-- STEP 2: UPDATE EXISTING STREAKS (FIXED UUID COMPARISON)
-- =====================================================
...
BEGIN
    -- Get template IDs - FIXED: Cast to UUID properly
    ...
    FOR r_streak IN
        ...
        WHERE s.template_id::text IN (v_hard_template_id::text, v_hard_plus_template_id::text)
    ...
    LOOP
        ...
        WHERE uh.streak_id = r_streak.streak_id
          ...
          AND th.template_id::text = r_streak.template_id::text; -- FIXED: Proper UUID comparison
        ...
        FOR r_template_habit IN
            ...
            FROM public.sz_template_habits th
            JOIN public.sz_habits h ON th.habit_id = h.id
            WHERE th.template_id::text = r_streak.template_id::text -- FIXED: Proper UUID comparison
        ...
        WHERE uh.streak_id = r_streak.streak_id
          ...
          AND NOT EXISTS (
            ...
            WHERE th_check.template_id::text = r_streak.template_id::text -- FIXED: Proper UUID comparison
...
SELECT 
    ...
    'Fixed UUID comparison error' as fix1,
    'Check-in function fixed (ambiguous column error resolved)' as fix2,
    'All existing streaks updated to match template definitions' as fix3,
    'Core habits properly marked and points calculated correctly' as fix4,
